...
async function handleRequest(req, res) {
  ...
  if (method === 'GET' && (path_ === '/' || path_ === '/index.html')) {
    const html = fs.readFileSync('./index-3.html');
    res.writeHead(200, { 'Content-Type': 'text/html' });
    return res.end(html);
  }

  // Serve any game HTML file: /xo.html, /snake.html, etc.
  if (method === 'GET' && path_.endsWith('.html') && path_ !== '/index-3.html') {
    const safeName = path_.replace(/[^a-zA-Z0-9_\-\.]/g, '');
    ...
    return send(res, 404, { ok: false, error: 'Game not found.' });
  }
...
route('POST', '/api/lobby/join', async (req, res) => {
  ...
  if (!lobby) {
    const lobbyCode = crypto.randomBytes(3).toString('hex').toUpperCase(); // e.g. "A3F9C2"
    const result = await lobbies.insertOne({
      ...
      is_private: req.body.private ? 1 : 0,
      created_at: new Date(),
      started_at: null,
      ended_at:   null,
    });
  ...
  const lobbyFresh = await lobbies.findOne({ _id: lobby._id });
  ok(res, {
    lobby_id:    lobby._id.toString(),
    lobby_code:  lobbyFresh.lobby_code || '',
    game_id,
    stake:       game.stake,
...
// POST /api/lobby/join-by-code  { code }
route('POST', '/api/lobby/join-by-code', async (req, res) => {
  const { code } = req.body;
  if (!code) return fail(res, 'Invite code required.');

  const lobby = await lobbies.findOne({ lobby_code: code.toUpperCase().trim(), status: 'waiting' });
  if (!lobby) return fail(res, 'Invalid or expired invite code.');

  const game = await games.findOne({ game_id: lobby.game_id, is_active: 1 });
  if (!game) return fail(res, 'Game not found or inactive.');

  const user = await users.findOne({ _id: req.user._id }, { projection: { coins: 1 } });
  if (user.coins < game.stake) return fail(res, `Not enough coins. Need ${game.stake}, you have ${user.coins}.`);

  const already = await lobbyMembers.findOne({ lobby_id: lobby._id, user_id: req.user._id });
  if (already) return fail(res, 'You are already in this lobby.');

  const memberCount0 = await lobbyMembers.countDocuments({ lobby_id: lobby._id });
  if (memberCount0 >= game.max_players) return fail(res, 'Lobby is full.');

  await users.updateOne({ _id: req.user._id }, { $inc: { coins: -game.stake } });
  await recordTx(req.user._id, -game.stake, 'stake_deducted', `Stake for ${game.name}`);
  await lobbies.updateOne({ _id: lobby._id }, { $inc: { pot: game.stake } });
  await lobbyMembers.insertOne({ lobby_id: lobby._id, user_id: req.user._id, stake: game.stake, joined_at: new Date() });
  ...
  if (memberCount >= game.min_players) {
    await lobbies.updateOne({ _id: lobby._id }, { $set: { status: 'active', started_at: new Date() } });
    started = true;
    broadcastToLobby(lobby._id.toString(), { type: 'match_start', lobby_id: lobby._id.toString(), game_id: lobby.game_id });
  } else {
    broadcastToLobby(lobby._id.toString(), { type: 'player_joined', count: memberCount, needed: game.min_players });
  }
...
// GET /api/users/search?q=username  — find a user by username for invite
route('GET', '/api/users/search', async (req, res) => {
  const url = new URL(req.url, 'http://localhost');
  const q   = (url.searchParams.get('q') || '').trim();
  if (q.length < 2) return fail(res, 'Search query too short.');

  const found = await users.find({
    username: { $regex: q, $options: 'i' },
    is_admin: 0,
  }).project({ username: 1, display_name: 1, avatar: 1, _id: 0 }).limit(10).toArray();
...
route('GET', /^\/api\/lobby\/(?<lobby_id>[^/]+)$/, async (req, res) => {
  ...
  // Sort by joined_at so members[0] is reliably the player who joined first (assigned X).
  const memberDocs = await lobbyMembers.find({ lobby_id: lobbyObjId }).sort({ joined_at: 1 }).toArray();
  const userIds     = memberDocs.map(m => m.user_id);
  const userDocs     = await users
    .find({ _id: { $in: userIds } })
    .project({ username: 1, display_name: 1, avatar: 1 })
    .toArray();

  // Mongo's $in does NOT preserve the order of userIds, so re-map results back
  // into joined_at order using memberDocs as the source of truth for ordering.
  const userById    = new Map(userDocs.map(u => [u._id.toString(), u]));
  const memberUsers = memberDocs
    .map(m => userById.get(m.user_id.toString()))
    .filter(Boolean)
    .map(({ username, display_name, avatar }) => ({ username, display_name, avatar }));
...
function handleWsMessage(socket, lobbyId, user, msg) {
  if (msg.type === 'chat') {
    ...
    broadcastToLobby(lobbyId, { type: 'chat', ...stored });
  } else if (msg.type === 'xo_move') {
    // Broadcast the move to ALL players in the lobby (including sender — client ignores own moves)
    if (typeof msg.index === 'number' && msg.symbol && msg.player) {
...
(async () => {
  ...
  // Server-side keepalive — ping all WS clients every 30s
  // Prevents Render/proxies from dropping idle connections
  setInterval(() => {
    Object.values(wsClients).forEach(room => {
      room.forEach(ws => {
        if (ws.readyState === 1) {
          try {
            ws.write(encodeFrame(JSON.stringify({ type: 'pong' })));
          } catch (e) { /* socket dead, will be cleaned on close event */ }
        }
      });
    });
  }, 30000);
