...
route('GET', /^\/api\/lobby\/(?<lobby_id>[^/]+)$/, async (req, res) => {
  ...
  // Sort by joined_at so members[0] is always the first player (X), members[1] is second (O)
  const memberDocs = await lobbyMembers.find({ lobby_id: lobbyObjId }).sort({ joined_at: 1 }).toArray();
  const memberUsers = [];
  for (const doc of memberDocs) {
    const u = await users.findOne({ _id: doc.user_id }, { projection: { username: 1, display_name: 1, avatar: 1, _id: 0 } });
